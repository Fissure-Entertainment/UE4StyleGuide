# [Fissure Entertainment](https://fissureentertainment.com) Unreal Engine 4 Style Guide() {

*A mostly reasonable approach to Unreal Engine 4 project management and style*
...
`Prefix` and `Suffix` are to be determined by the asset type through the following [Asset Name Modifier](#1.2-anm) tables.
...
| Level (Persistent)      |            | _Persistent|                                  |
| Level (Audio)           |            | _Audio     |                                  |
| Level (Lighting)        |            | _Lighting  |                                  |
| Level (Geometry)        |            | _Geo       |                                  |
| Level (Gameplay)        |            | _Gameplay  |                                  |
| Level (*)               |            | _*         | Whatever you feel is appropriate |
| Blueprint               | BP_        |            |                                  |
| Material                | M_         |            |                                  |
...
| Particle System         | PS_        |            |                                  |
| Widget Blueprint        | WB_ or WBP_|            | Pick only one. Prefer WBP_.      |
...
| Structure               | F or S     |            | No underscore. Prefer S.         |
| Widget Blueprint        | WB_ or WBP_|            | Pick only one. Prefer WBP_.      |
...
| Subsurface Profile      | SP_ or SSP_|            | Pick only one. Prefer SSP_.      |
...
| Texture (Ambient Occlusion) | T_     | _O or _AO  | Pick only one. Prefer _AO.       |
| Texture (Bump)          | T_         | _B         |                                  |
...
Never locate a project on the Desktop.
...
All of a project's assets should exist in a folder named after the project. For example, if your project is named 'Project Revive', _all_ of it's content should exist in `Content/ProjectRevive`.
...
Try to not use verbs such as `bRunning`. Verbs tend to lead to complex states. If a verb like that describes the variable better it may need to be an Enumeration instead?
...
It is good practice to always provide an Tooltip description regardless of its Editableness. However not required it is recommended.
...
> You can define sub-categories by using the pipe character `|`, i.e. `Config|Animations` (do not apply spaces between the categories and the pipe character).
...
* Matthew Boatswain (mboatswain@fissureentertainment.com)

## License

Copyright 2016 Fissure Entertainment, Pty Ltd. All Rights Reserved.
